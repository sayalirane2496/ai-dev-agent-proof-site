...
import { PRODUCTS, PERI_PERI_FRIES_IMAGE, BK_CAFE_SHAKE_IMAGE } from '@/features/catalog/mock-fallback';
...
import type { ComboSide } from '@/features/catalog/catalog-context';
...
export const SmartComboBuilder: React.FC<SmartComboBuilderProps> = ({
  onAddComboToCart,
  products = PRODUCTS,
  comboSides = [],
}) => {
  const comboBurgers = products.filter((p) =>
    ['prod-whopper-veg', 'prod-whopper-chicken', 'prod-crispy-veg', 'prod-paneer-royale'].includes(p.id),
  );
  ...
  const friesFromApi = comboSides
    .filter((s) => s.kind === 'fries')
    .map((s) => ({ name: s.name, priceDelta: s.price_delta_inr, image: s.image_path || PERI_PERI_FRIES_IMAGE, isHot: s.is_hot }));
  const drinksFromApi = comboSides
    .filter((s) => s.kind === 'drink')
    .map((s) => ({ name: s.name, priceDelta: s.price_delta_inr, image: s.image_path || BK_CAFE_SHAKE_IMAGE, tag: s.tag || undefined }));

  const comboFries = friesFromApi.length ? friesFromApi : defaultFries;
  const comboDrinks = drinksFromApi.length ? drinksFromApi : defaultDrinks;
