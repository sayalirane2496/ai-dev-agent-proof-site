...
interface CheckoutModalProps {
  ...
  onOrderSuccess: (orderId: string, address: string) => void;
}
...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  const [step, setStep] = useState<1 | 2 | 3>(1); // 1: Delivery Address & Contact, 2: Payment, 3: Processing
  ...
  const handlePlaceOrder = () => {
    ...
    setTimeout(() => {
      ...
      const randomOrderId = `BK-IN-${Math.floor(10000 + Math.random() * 90000)}`;
      onOrderSuccess(randomOrderId, fullAddress);
      onClose();
    }, 1200);
  };
            ...
            {step === 2 && (
              <div className="space-y-6">
                ...
                <div className="p-3 bg-[#FAF5ED] rounded-xl flex items-center gap-2 text-xs text-[#59483F] font-semibold border border-[#EBE1D0]">
                  ...
                  <span>Secure 256-bit bank encryption · 100% Buyer Protection</span>
                </div>
                ...
                >
                  {isProcessing ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span>Securing Flame-Grilled Order...</span>
                    </div>
                  ) : (
                    <>
